...
from repository import initialize_database, get_security, get_vms_by_group


def get_inputs(Session):
    security = get_security(Session)

    all_VMs = get_vms_by_group("AZURE", Session)
    if security is None:
